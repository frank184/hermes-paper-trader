...
Chart endpoints write both JSON and self-contained HTML artifacts into `./artifacts` through the orchestrator container's `/artifacts` mount. The same host folder is mounted into Hermes Workspace at `workspace/artifacts`, so `chart_symbol` and `chart_backtest` responses include `workspace_artifact_paths.html` for a browser-viewable chart.
