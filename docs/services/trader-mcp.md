...
`get_market_bars`, `chart_symbol`, `chart_backtest`, `get_symbol_report`, and `get_portfolio_report` fetch data through the orchestrator, persist useful market rows, and return structured chart/report artifacts. Chart tools also return `chat_markdown_image`; Hermes should include that value verbatim in the assistant response when you want the chart rendered inline in chat.
