...
`get_market_bars`, `chart_symbol`, `chart_backtest`, `get_symbol_report`, and `get_portfolio_report` fetch data through the orchestrator, persist useful market rows, and return structured chart/report artifacts. Chart tools return `assistant_response_markdown`; Hermes should show that by default because it already includes the PNG chart and summary. Workspace rewrites the returned `MEDIA:/...` marker into a browser-safe image URL.
